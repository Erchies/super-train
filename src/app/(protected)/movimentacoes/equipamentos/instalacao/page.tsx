...
function InstalacaoForm() {
  ...
  // Todas as posições possíveis para o contexto nomeEquipamento+serie selecionado
  ...
  useEffect(() => {
    Promise.all([
      fetch("/api/equipamentos?status=EM_ESTOQUE").then((r) => r.json()),
  ...
  // Quando equipamento + TUE selecionados: busca posições por nomeEquipamento + serie
  useEffect(() => {
    if (!equipamentoId || !tueId) {
    ...
    const tue = tues.find((t) => t.id === tueId);
    if (!equip || !tue) return;
  ...
  }, [equipamentoId, tueId, equipamentos, tues]);
  ...
  const tuesFiltrados = equipSel
    ? tues.filter((t) => equipSel.compatibilidade === "AMBAS" || t.serie === equipSel.compatibilidade)
    : tues;
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const result = await instalarEquipamentoAction({
      equipamentoId,
      tueId,
      carro,
      posicaoInstalada,
  ...
  return (
    <div>
      <PageHeader
        title="Instalar Equipamento em TUE"
        breadcrumbs={[{ label: "Movimentações" }, { label: "Instalação em TUE" }]}
      />
      ...
      <div className="max-w-2xl rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            {equipSel && (
              <p className="mt-1 text-xs text-gray-500">
                Compatibilidade: {equipSel.compatibilidade === "AMBAS" ? "Ambas as séries" : equipSel.compatibilidade}
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <select
              value={tueId}
              onChange={(e) => { setTueId(e.target.value); setCarro(""); setPosicaoInstalada(""); }}
              required
              className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              <option value="">- Selecione o TUE -</option>
              {tuesFiltrados.map((t) => (
                <option key={t.id} value={t.id}>
                  TUE {t.numero} ({t.serie === "SERIE_100" ? "Série 100" : "Série 200"}){t.descricao ? ` - ${t.descricao}` : ""}
                </option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            {posicoesDisponiveis.length > 0 ? (
              ...
              <input
                ...
                required
                placeholder={
                  equipamentoId && tueId && carro
                    ? "Nenhuma posição cadastrada — informe manualmente"
                    : "Selecione Equipamento, TUE e Carro primeiro"
                }
                disabled={!equipamentoId || !tueId || !carro}
                className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-50"
              />
            )}
            {equipamentoId && tueId && carro && posicoesDisponiveis.length === 0 && (
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {saving ? "Instalando..." : "Confirmar Instalação"}
            </button>
            <button
              type="button"
              onClick={() => router.back()}
