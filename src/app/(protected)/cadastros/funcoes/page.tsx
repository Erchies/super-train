...
export default function FuncoesPage() {
  ...
  async function load() {
    const res = await fetch("/api/cadastros/funcoes");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const url = editItem ? `/api/cadastros/funcoes/${editItem.id}` : "/api/cadastros/funcoes";
    const res = await fetch(url, { method: editItem ? "PUT" : "POST", body: fd });
    const data = await res.json();

    if (data.success) {
      setSuccess(editItem ? "Função atualizada!" : "Função criada!");
      setShowForm(false);
      await load();
    } else {
      setError(data.error);
  ...
  async function toggleAtivo(item: Item) {
    ...
    await fetch(`/api/cadastros/funcoes/${item.id}`, { method: "PUT", body: fd });
    await load();
  }
  ...
  return (
    <div>
      <PageHeader
        title="Funções"
        description="Funções de instalação para equipamentos"
        breadcrumbs={[{ label: "Cadastros" }, { label: "Funções" }]}
        action={<button onClick={openNew} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-blue-700"><Plus className="w-4 h-4" /> Nova Função</button>}
      />
      ...
      {showForm && (
        <div className="bg-white border border-gray-200 rounded-lg p-6 mb-6 shadow-sm">
          <h2 className="text-lg font-semibold mb-4">{editItem ? "Editar Função" : "Nova Função"}</h2>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Código <span className="text-red-500">*</span></label>
              <input value={codigo} onChange={(e) => setCodigo(e.target.value)} required placeholder="ex: COMP-TRAC" className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 uppercase" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Descrição <span className="text-red-500">*</span></label>
              <input value={descricao} onChange={(e) => setDescricao(e.target.value)} required placeholder="ex: Computador de Tração" className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {items.map((item) => (
                <tr key={item.id} className={item.ativo ? "" : "opacity-50"}>
                  ...
                  <td className="px-4 py-3">{item.ativo ? <span className="flex items-center gap-1 text-green-700 text-xs"><CheckCircle className="w-3 h-3" /> Ativo</span> : <span className="flex items-center gap-1 text-red-500 text-xs"><XCircle className="w-3 h-3" /> Inativo</span>}</td>
              ...
              {items.length === 0 && <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-500">Nenhuma função cadastrada.</td></tr>}
            </tbody>
          </table>
        )}
      </div>
